...
app
  ...
  .then(() => {
    ...
    globalShortcut.register("CommandOrControl+Shift+F10", () => {
      ...
      mainWindow?.webContents.send("segra:save-replay-buffer", { source: "hotkey-legacy" })
    })

    // Medal-style instant replay hotkey (default F8). Segra owns re-registration
    // when the user changes it in Settings; this is just the boot default.
    try {
      if (!globalShortcut.isRegistered("F8")) {
        globalShortcut.register("F8", () => {
          mainWindow?.webContents.send("segra:save-replay-buffer", { source: "hotkey", hotkey: "F8" })
          mainWindow?.webContents.send("clips:save-request")
        })
      }
    } catch (err) {
      console.error("[ChibangaRx]: F8 replay hotkey failed:", err)
    }
