...
function clearChibangaRxCache(): ClearCacheResult {
  ...
  try {
    ...
    if (fs.existsSync(scriptsPath)) {
      ...
      scriptsCleared = true
      console.log("ChibangaRx scripts directory files cleared successfully.")
    } else {
      console.warn("ChibangaRx scripts directory does not exist.")
    ...
    console.error("Failed to clear ChibangaRx scripts or logs directory:", error)
    return { success: false, error: error.message }
  }
}
...
function Show-InstallerGUI {
    ...
    $form.Text = "ChibangaRx: Winget Installer"
    $form.Size = New-Object System.Drawing.Size(600,400)
    $form.StartPosition = "CenterScreen"

    $label = New-Object System.Windows.Forms.Label
    $label.Text = "Welcome! ChibangaRx needs Winget to install apps."
    $label.AutoSize = $true
    ...
    [void]$powershell.AddScript({
        ...
        try {
            ...
            if ($TestMode -or -not $wingetInstalled) {
                $result.Messages += "Winget not found. Installing for ChibangaRx..."
                ...
                $result.Messages += "Winget is already installed. ChibangaRx is ready to install apps!"
                $result.Success = $true
            }
        } catch {
...
if ($TestMode -or -not (Check-Winget)) {
    ...
    Write-Output "Winget is already installed. ChibangaRx can install apps!"
}
`
