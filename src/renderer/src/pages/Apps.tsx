...
function Apps() {
    ...
    reader.onload = (e: ProgressEvent<FileReader>) => {
      try {
        ...
        if (Array.isArray(parsed)) {
          ...
          toast.error(t("settings.appsInvalidImport"))
        }
      } catch {
        toast.error(t("settings.appsParseError"))
      } finally {
        event.target.value = ""
      }
    }
  ...
  const installWinget = async (): Promise<void> => {
    ...
    try {
      ...
      toast.success(t("apps.appsWingetInstallComplete"))
      await checkWinget()
    } catch (error) {
      console.error("Error installing Winget:", error)
      toast.error(t("apps.appsWingetInstallError"))
    } finally {
  ...
  const installChocolatey = async (): Promise<void> => {
    ...
    try {
      ...
      toast.success(t("apps.appsChocolateyInstallComplete"), {
        autoClose: false,
      })
      await checkChocolatey()
    } catch (error) {
      console.error("Error installing Chocolatey:", error)
      toast.error(t("apps.appsChocolateyInstallError"))
    } finally {
  ...
  useEffect(() => {
    const loadApps = async () => {
      try {
        ...
        toast.error(t("apps.appsFetchError"))
    ...
    const handleInstallError = () => {
      toast.error(t("apps.appsInstallationError"))
    }
  ...
  return (
    <>
            ...
            <div className="space-y-3">
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  {wingetInstalled && (
                    <span className="text-green-500 text-sm font-medium">{t("apps.appsInstalledText")}</span>
                  )}
                  {!wingetInstalled && !wingetChecking && (
                    <span className="text-amber-500 text-sm font-medium">{t("apps.appsNotInstalledText")}</span>
                  )}
                </div>
              </button>
                ...
                {importedApps.map((id) => {
                  ...
                  return (
                    <li key={id} className="flex items-center gap-2 text-chibangarx-text">
                      ...
                      {app ? app.name : t("apps.appsUnknown", { id })}
                    </li>
                  )
                })}
              </ul>
      ...
      <RootDiv>
          <LargeInput
            icon={Search}
            placeholder={t("apps.appsSearch", { count: filteredApps.length })}
            value={search}
              ...
              >
                {wingetInstalling ? (
                  <>
                    ...
                    {t("apps.appsInstalling")}
                  </>
                ) : wingetChecking ? (
                  <>{t("apps.appsChecking")}</>
                ) : (
                  <>
                    <Download size={18} /> {t("apps.appsInstallWinget")}
                  </>
                )}
              </Button>
            </div>
            ...
            <div className="ml-auto">
              ...
              >
                {chocolateyInstalling ? (
                  ...
                  <>
                    <Download size={18} /> {t("apps.appsInstallChocolatey")}
                  </>
                )}
              </Button>
            </div>
        ...
        <div className="flex gap-3 mt-5 w-auto ml-1 mr-1">
          {selectedApps.length > 0 && (
            ...
            >
              {selectedApps.length} {t("apps.appsSelectedCount", { count: selectedApps.length })}
            </Button>
          )}
          <Button
        ...
        <div className="flex flex-row gap-2 items-center">
          ...
          <div className="ml-auto flex gap-2 items-center">
            ...
            <Dropdown
              ...
              onChange={(value) => {
                const newSource = value === t("settings.chocolatey") ? "Chocolatey" : "Winget"
                setSource(newSource as "Chocolatey" | "Winget")
              }}
            />
          </div>
        </div>
