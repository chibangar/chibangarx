...
export function ScrollProvider({ children }: { children: React.ReactNode }) {
  const [scrollPositions, setScrollPositions] = useState<ScrollPositions>({
    sessions: 0, clips: 0, highlights: 0, replayBuffer: 0,
  });

  const setScrollPosition = useCallback((page: keyof ScrollPositions, position: number) => {
    setScrollPositions((prev) => ({ ...prev, [page]: position }));
  }, []);

  const contextValue = useMemo(() => ({ scrollPositions, setScrollPosition }), [scrollPositions, setScrollPosition]);
