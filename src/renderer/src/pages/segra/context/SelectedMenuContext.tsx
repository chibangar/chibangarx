...
export const SelectedMenuProvider = ({ children }: { children: ReactNode }) => {
  const [selectedMenuState, setSelectedMenuState] = useState<string>('Full Sessions');
...
export const useSelectedMenu = () => {
  ...
  if (!context) throw new Error('useSelectedMenu must be used within a SelectedMenuProvider');
  return context;
};
